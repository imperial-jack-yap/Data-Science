...
pgpd <- function(x, mu, sigma, psi){
  if (x < mu){
    return("Error: X less than mu")
  }
  if (psi != 0){
    max_val <- 1 + psi*(x - mu)/sigma
    return (1 - max(max_val, 0)^(-1/psi))
  }else{
    return (1 - exp(-(x - mu)/sigma))
  }
}
...
for (id in ids){
  ...
  sigma <- param$sig_sim
  psi <- param$xi_sim
  plot(samples$value, pgpd(samples$value, mu, sigma, psi))
}
